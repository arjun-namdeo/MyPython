...
_aux_logger = logging.getLogger('media_browser.aux')
...
confFileName = 'media_browser.cfg'
    ...
    def __init__(self, defaults=None, confFile=None, *args, **kwds):
        ...
        *confFile* -- the file (or list of files) to parse."""
        ...
        for fname in confFile:
            ...
            if self.has_section('media_browser'):
                break
...
def IMDb(accessSystem=None, *arguments, **keywords):
    ...
    if accessSystem is None or accessSystem in ('auto', 'config'):
        try:
            ...
            kwds = cfg_file.getDict('media_browser')
            ...
            import logging
            logging.getLogger('media_browser').warn('Unable to read configuration' \
    ...
    if 'loggingConfig' in keywords:
        ...
        try:
            ...
            logging.getLogger('media_browser').warn('unable to read logger ' \
                                            'config: %s' % e)
    if accessSystem in ('httpThin', 'webThin', 'htmlThin'):
        logging.warn('httpThin was removed since media_browser 4.8')
        accessSystem = 'http'
    if accessSystem in ('http', 'web', 'html'):
        ...
        # The local access system was removed since media_browser 4.2.
        raise IMDbError('the local access system was removed since media_browser 4.2')
...
class IMDbBase:
    ...
    # Top-level logger for media_browser.
    _imdb_logger = logging.getLogger('media_browser')
